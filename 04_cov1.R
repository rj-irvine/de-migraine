...
# Filter contacts on date + type, then collect. patpop_matched_obs() joins this
# to the matched cohort locally.
temp <- contact |>
  right_join(
  ...
  filter(
    event_date >= StartDate &
      # Drop referral-type contacts ("R") from the GP-visit count.
      !contact_type_code == "R"
  ) |>
  collect()
